...
for /F %%i in ('docker ps --all --quiet --filter "ancestor=spiritual-domain-example-mvc-war:v1.0" --filter "status=running"') do (
...
for /F %%i in ('docker ps --all --quiet --filter "ancestor=spiritual-domain-example-mvc-war:v1.0" --filter "status=exited"') do (
...
for /F %%i in ('docker images --all --quiet spiritual-domain-example-mvc-war:v1.0') do (
